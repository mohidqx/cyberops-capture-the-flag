...
export const TerminalHeader = ({ title, icon: Icon, color = "text-primary", actions }: { title: string; icon: any; color?: string; actions?: React.ReactNode }) => (
  <div className="flex items-center justify-between px-4 py-2.5 border-b border-border/30 bg-background/60">
    <div className="flex items-center gap-2.5">
      ...
      <span className="font-mono text-xs uppercase tracking-[0.15em] text-muted-foreground">{title}</span>
    </div>
  ...
  >
    ...
    {!collapsed && (
      <>
        ...
        {badge != null && badge > 0 && (
          <span className={`ml-auto text-[9px] px-1.5 py-0.5 rounded-full ${badgeColor}/20 text-foreground font-bold`}>{badge}</span>
        )}
      </>
    )}
...
export const StatCard = ({ icon: Icon, label, value, color, desc, total, delay = 0 }: {
  ...
  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay }}>
    <div className={`group relative p-3 rounded-lg border border-border/20 bg-background/40 hover:bg-background/60 hover:border-${color}/30 transition-all duration-300 cursor-default`}>
      <div className="flex items-center gap-2 mb-1">
        <Icon className={`w-3.5 h-3.5 text-${color}`} />
        <span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground truncate">{label}</span>
      </div>
      <div className={`text-lg font-display font-black text-${color}`}>{value}</div>
      <div className="text-[9px] text-muted-foreground/60 font-mono mt-0.5">{desc}</div>
      {total != null && (
        <div className="mt-1.5 h-1 rounded-full bg-border/30 overflow-hidden">
          <motion.div className={`h-full rounded-full bg-${color}`} initial={{ width: 0 }} animate={{ width: `${(Number(value) / total) * 100}%` }} transition={{ delay: delay + 0.3, duration: 0.6 }} />
        </div>
      )}
    </div>
  </motion.div>
...
export const ConfigToggle = ({ label, desc, checked, onChange, color = "primary" }: {
  ...
  <div className="flex items-center justify-between p-3 rounded-lg border border-border/20 bg-background/40 hover:bg-background/50 transition-colors">
    <div>
      <div className="font-mono text-sm font-semibold">{label}</div>
      <div className="text-[10px] text-muted-foreground">{desc}</div>
    </div>
    <div className="flex items-center gap-2">
      <div className={`w-1.5 h-1.5 rounded-full ${checked ? `bg-${color} animate-pulse` : "bg-muted-foreground/30"}`} />
      <input type="checkbox" checked={checked} onChange={e => onChange(e.target.checked)}
        className="w-9 h-5 rounded-full appearance-none cursor-pointer bg-muted checked:bg-primary transition-colors relative after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:w-4 after:h-4 after:rounded-full after:bg-white after:transition-transform checked:after:translate-x-4"
      />
    </div>
  </div>
);
...
export const ConfigInput = ({ label, desc, value, onChange, type = "text", placeholder }: {
  ...
  <div className="p-3 rounded-lg border border-border/20 bg-background/40">
    ...
    <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
      className="w-full h-8 px-3 rounded-md border border-border/30 bg-background/60 text-sm font-mono text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary/50 transition-colors"
    />
  </div>
);
...
export const SectionLabel = ({ label, color = "primary" }: { label: string; color?: string }) => (
  <div className="flex items-center gap-2 pt-2 pb-1">
    ...
    <span className={`text-[9px] font-mono uppercase tracking-[0.2em] text-${color}/50`}>{label}</span>
    <div className={`h-px flex-1 bg-${color}/20`} />
  </div>
);
...
export const ActionBtn = ({ icon: Icon, label, color = "primary", onClick, disabled }: {
  ...
  <button onClick={onClick} disabled={disabled}
    className={`flex items-center gap-2 p-2.5 rounded-lg border border-border/20 bg-background/30 hover:border-${color}/30 hover:bg-${color}/5 transition-all text-xs font-mono uppercase tracking-wider text-muted-foreground hover:text-${color} disabled:opacity-30 disabled:pointer-events-none`}
  >
...
export const LogLine = ({ time, msg, color = "text-primary", delay = 0 }: { time: string; msg: string; color?: string; delay?: number }) => (
  <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay }}
    ...
    <span className="text-muted-foreground/50 shrink-0 w-12 text-right">{time}</span>

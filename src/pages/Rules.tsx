...
const Rules = () => {
  const { content, loading } = usePageContent("page_rules");
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
          <div className="text-center mb-12">
            ...
            <h1 className="font-display text-4xl md:text-5xl font-bold text-foreground mb-4">
              {page?.title || "Competition"} <span className="text-gradient">Rules</span>
            </h1>
            <p className="text-muted-foreground">{page?.subtitle || "Fair play for all participants"}</p>
          </div>
            ...
            {page?.sections?.length ? (
              ...
              <>
                <section className="p-6 rounded-lg border border-border bg-card/50">
                  <h2 className="font-display text-xl font-bold text-foreground mb-4">Allowed</h2>
                  <p className="text-muted-foreground leading-relaxed">Using any publicly available tools and resources. Collaborating with teammates. Writing custom scripts and exploits.</p>
                </section>
                <section className="p-6 rounded-lg border border-border bg-card/50">
                  <h2 className="font-display text-xl font-bold text-foreground mb-4">Not Allowed</h2>
                  <p className="text-muted-foreground leading-relaxed">Sharing flags with other teams. Attacking the CTF infrastructure. Denial of service attacks. Using multiple accounts.</p>
                </section>
              </>
            )}
          </div>
